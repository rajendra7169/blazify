...
import com.blazify.innertube.pages.ArtistSection
...
fun serializeArtistPage(
    ...
    artist: com.blazify.innertube.models.ArtistItem? = null,
): String {
...
fun CachedArtistPage.toArtistPage(): com.blazify.innertube.pages.ArtistPage {
    return com.blazify.innertube.pages.ArtistPage(
        artist = artist?.let { cached ->
            com.blazify.innertube.models.ArtistItem(
                id = cached.id,
                title = cached.title,
...
private fun CachedItem.toYTItem(): YTItem {
    return when (this) {
        is CachedItem.Song -> SongItem(
            ...
            thumbnail = thumbnail,
            explicit = explicit,
            artists = artists.map { com.blazify.innertube.models.Artist(it.name, it.id) },
            album = album?.let { com.blazify.innertube.models.Album(it.name, it.id) },
            duration = duration,
            setVideoId = videoId,
        )
        is CachedItem.Album -> AlbumItem(
            ...
            thumbnail = thumbnail,
            explicit = explicit,
            artists = artists.map { com.blazify.innertube.models.Artist(it.name, it.id) }.ifEmpty { null },
            year = year,
        )
        is CachedItem.Playlist -> PlaylistItem(
            ...
            author = author?.let { com.blazify.innertube.models.Artist(it.name, it.id) },
        ...
        is CachedItem.Episode -> EpisodeItem(
            ...
            explicit = explicit,
            author = author?.let { com.blazify.innertube.models.Artist(it.name, it.id) },
            podcast = album?.let { com.blazify.innertube.models.Album(it.name, it.id) },
            duration = duration,
            endpoint = null,
        )
    }
